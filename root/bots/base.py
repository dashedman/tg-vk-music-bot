...
class BaseBot:
    ...
    def create_self_signed_cert(self):
        ...
        cert.get_subject().CN = self.config['network']['domen']  # указываем свои данные
        ...
        with open(os.path.join(self.config['ssl']['dir'], self.config['ssl']['cert_filename']), "w") as f:
            f.write(crypto.dump_certificate(crypto.FILETYPE_PEM, cert).decode("ascii"))

        with open(os.path.join(self.config['ssl']['dir'], self.config['ssl']['key_filename']), "w") as f:
